...
if (!function_exists('ops_follow_curl_location')) {
    function ops_follow_curl_location(&$ch)
    {
        $max_redir = 5;
...
if (!function_exists('ops_curl')) {
    function ops_curl($url, $show_headers = 1, $retry = 0)
    {
        if ($retry > 5) {
        ...
        if (preg_match("|Location: (https?://\S+)|", $result, $m)) {
            return ops_curl($m[1], $show_headers, $retry + 1);
        }
